...
impl PermissionManager {
  ...
  pub fn get(&self, user_id: &str) -> Permission {
    let conn = self.db.get().unwrap();
    conn
      .query_row(
  ...
  pub fn set(&self, user_id: &str, perm: Permission) {
    let conn = self.db.get().unwrap();
    conn
      .execute(
        ...
        params![user_id, perm.bits()],
      )
      .unwrap();
  }

  pub fn grant(&self, user_id: &str, perm: Permission) {
    let current = self.get(user_id);
    self.set(user_id, current | perm);
  }

  pub fn revoke(&self, user_id: &str, perm: Permission) {
    let current = self.get(user_id);
    self.set(user_id, current - perm);
  }

  pub fn has(&self, user_id: &str, perm: Permission) -> bool {
